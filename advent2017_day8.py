...
def evaluate_condition(register_dict, register, comparison, number):
    register_value = register_dict.get(register, 0)

    if comparison == "<":
        return register_value < number
    elif comparison == "<=":
        return register_value <= number
    elif comparison == ">":
        return register_value > number
...
def process_instruction(register_dict, register, operation, value, cmpreg, cmpop, cmpvalue):
    global all_time_max
    register_value = register_dict.get(register, 0)
    if operation == "dec":
        value *= -1

    if evaluate_condition(register_dict, cmpreg, cmpop, cmpvalue):
        register_value += value
        if register_value > all_time_max:
            all_time_max = register_value
        register_dict[register] = register_value

line_match = re.compile(r'([a-z]+) ([a-z]+) ([0-9-]+) if ([a-z]+) ([^ ]+) ([0-9-]+)')

for line in INPUT.split("\n"):
    match = line_match.match(line)
    process_instruction(registers, match.group(1), match.group(2), int(match.group(3)),
                        match.group(4), match.group(5), int(match.group(6)))

print("Final values: %r" % registers)
max_register = max(registers.iterkeys(), key=lambda x: registers[x])
print("Max value: %d in register %s" % (registers[max_register], max_register))
print("All time max value: %d" % all_time_max)
