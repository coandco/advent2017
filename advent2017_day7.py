...
INPUT = """vpryah (310) -> iedlpkf, epeain
aqfemi (38)
...
class Program:
    def __init__(self, name, weight=0, children=None):
        self.parent = None
        self.name = name
...
def calc_weights(program_name):
    for child in tower[program_name].children:
        tower[program_name].combined_weight += calc_weights(child)

    tower[program_name].combined_weight += tower[program_name].weight
    return tower[program_name].combined_weight


def find_imbalance(base):
    current_program = base
    off_by = None
    while True:
        children_weights = {}
        for child in tower[current_program].children:
            if tower[child].combined_weight not in children_weights:
                children_weights[tower[child].combined_weight] = []
            children_weights[tower[child].combined_weight].append(child)
        if len(children_weights) == 1:
            return current_program, off_by
        else:
...
line_matcher = re.compile(r'^([a-z]+) \((\d+)\)(?: -> (.*))?')
for line in INPUT.split("\n"):
    match = line_matcher.match(line)
    name, weight, children = match.group(1), match.group(2), match.group(3)
    tower[match.group(1)] = Program(name, int(weight), children.split(", ") if children is not None else None)

for program in tower:
    for child in tower[program].children:
        tower[child].parent = tower[program].name

current_program = tower.keys()[0]
while tower[current_program].parent is not None:
    print("Testing %s, parent %r" % (current_program, tower[current_program].parent))
    current_program = tower[current_program].parent

print("Bottom of tower is %s, as evidenced by parent %r" % (current_program, tower[current_program].parent))

calc_weights(current_program)

bad_program, off_by = find_imbalance(current_program)
print("Bad program %s has weight %d" % (bad_program, tower[bad_program].weight))
print("Weight needs to change by %d, so correct bad program weight is %d" % (off_by, tower[bad_program].weight + off_by))
