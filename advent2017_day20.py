...
p=<-2667,-1543,1410>, v=<-382,-221,200>, a=<29,16,-16>""".split('\n')


def calculate_magnitude(three_dee_vector):
    return abs(three_dee_vector[0]) + abs(three_dee_vector[1]) + abs(three_dee_vector[2])


def detect_collisions(particles):
    positions = {}
    for i, particle in enumerate(particles):
        if tuple(particle['p']) not in positions.keys():
            positions[tuple(particle['p'])] = []
        positions[tuple(particle['p'])].append(i)

    collided_particles = []
    for position in positions:
        if len(positions[position]) > 1:
            collided_particles.extend(positions[position])

    return sorted(collided_particles, reverse=True)


def run_sim_tick(particles):
...
for line in INPUT:
    ...
    position = [int(x) for x in components[0][3:-1].split(',')]
    velocity = [int(x) for x in components[1][3:-1].split(',')]
    acceleration = [int(x) for x in components[2][3:-1].split(',')]
    PARTICLES.append({'p': position, 'v': velocity, 'a': acceleration})

lowest_accel = None
for i, particle in enumerate(PARTICLES):
    if lowest_accel is None or calculate_magnitude(particle['a']) < calculate_magnitude(PARTICLES[lowest_accel]['a']):
        lowest_accel = i

print("Lowest acceleration particle is in slot %d" % lowest_accel)
...
while True:
    ...
    print("Number of particles left: %d" % len(PARTICLES))
    PARTICLES = run_sim_tick(PARTICLES)
